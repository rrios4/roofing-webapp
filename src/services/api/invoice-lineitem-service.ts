...
export const createInvoiceLineItem = async (newInvoiceLineItemObject: InvoiceLineServiceInsert): Promise<InvoiceLineService> => {
...
export const deleteAllInvoiceLineItemsByInvoiceNumber = async (invoiceNumber: number): Promise<InvoiceLineService[]> => {
  const { data, error } = await supabase
    .from(TABLES.INVOICE_LINE_SERVICE)
    .delete()
...
export const deleteInvoiceLineItemById = async (deleteLineItemObject: { item_id: number }): Promise<InvoiceLineService> => {
  const { data, error } = await supabase
    .from(TABLES.INVOICE_LINE_SERVICE)
    .delete()

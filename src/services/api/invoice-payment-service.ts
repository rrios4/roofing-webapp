...
export const createInvoicePayment = async (newInvoicePaymentObject: InvoicePaymentInsert): Promise<InvoicePayment> => {
...
export const deleteInvoicePayment = async (deleteInvoicePaymentObject: { item_id: number }): Promise<InvoicePayment> => {
  const { data, error } = await supabase
    .from(TABLES.INVOICE_PAYMENT)
    .delete()
...
export const deleteAllInvoicePaymentsByInvoiceNumber = async (invoiceNumber: number): Promise<InvoicePayment[]> => {
  const { data, error } = await supabase
    .from(TABLES.INVOICE_PAYMENT)
    .delete()

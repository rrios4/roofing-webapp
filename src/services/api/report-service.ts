  ...
  try {
    ...
    const [
      ...
      supabase
        .from(TABLES.QUOTE)
        .select('*', { count: 'exact' })
        .eq('status_id', 3), // 3 = Rejected

      // Invoice statuses
      supabase
        .from(TABLES.INVOICE)
        .select('*', { count: 'exact' })
        .eq('invoice_status_id', 1), // 1 = Paid
      supabase
        .from(TABLES.INVOICE)
        .select('*', { count: 'exact' })
        .eq('invoice_status_id', 2), // 2 = Pending
      supabase
        .from(TABLES.INVOICE)
        .select('*', { count: 'exact' })
        .eq('invoice_status_id', 3), // 3 = Overdue
...
export const fetchInvoiceStatusTracking = async (): Promise<InvoiceStatusTracking> => {
  try {
    ...
    const possibleAmountColumns = ['amount', 'total', 'invoice_amount', 'total_amount', 'grand_total', 'invoice_total'];
    ...
    // Check for errors
    const results = [paidInvoicesResult, pendingInvoicesResult, overdueInvoicesResult, draftInvoicesResult];
    for (const result of results) {
      if (result.error) {

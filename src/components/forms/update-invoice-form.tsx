...
import { updateInvoiceFormSchema } from '../../validations/invoice-form-validations';
...
import { Invoice, InvoiceLineService } from '../../types/db_types';

type Props = {
  invoice: Invoice;
  invoiceLineItems?: InvoiceLineService[];
  onSuccess?: () => void;
};
...
export default function UpdateInvoiceForm({ invoice, invoiceLineItems, onSuccess }: Props) {
  ...
  const [billToSwitch, setBillToSwitch] = useState(!!invoice.bill_to);
  const [generalNoteSwitch, setGeneralNoteSwitch] = useState(!!invoice.private_note);
  const [measurementNoteSwitch, setMeasurementNoteSwitch] = useState(!!invoice.sqft_measurement);
  const [customerNoteSwitch, setCustomerNoteSwitch] = useState(!!invoice.cust_note);

  const form = useForm<z.infer<typeof updateInvoiceFormSchema>>({
    ...
    defaultValues: {
      ...
      line_items: invoiceLineItems?.map((item) => ({
        ...
        amount: item.amount,
        sq_ft: item.sq_ft || 0,
        fixed_item: item.fixed_item || true
      })) || [{ description: '', qty: 1, rate: 0, amount: 0, sq_ft: 0, fixed_item: true }],
      bill_from_street_address: invoice.bill_from_street_address || '150 Tallant St',
      bill_from_city: invoice.bill_from_city || 'Houston',
      bill_from_state: invoice.bill_from_state || 'TX',
      bill_from_zipcode: invoice.bill_from_zipcode || '77076',
      ...
      bill_to_zipcode: invoice.bill_to_zipcode || '',
      bill_to: invoice.bill_to || false,
  ...
  async function updateInvoice(updatedInvoice: any) {
  ...
  async function updateInvoiceLineItems(invoiceLineItems: any) {
    ...
    const { data, error } = await supabase.from(TABLES.INVOICE_LINE_SERVICE).insert(itemsToInsert);

    if (error) {
      console.error('Error updating invoice line items', error.message);
  ...
  const invoiceMutation = useMutation({
    ...
    onSuccess: () => {
      // Invalidate the main invoices list
      queryClient.invalidateQueries({ queryKey: ['invoices'] });
      // Invalidate the specific invoice by ID and invoice number
      queryClient.invalidateQueries({ queryKey: ['invoiceById', invoice.invoice_number] });
      queryClient.invalidateQueries({ queryKey: ['invoiceById'] });
      // Invalidate invoice status counts for the stats cards
  ...
  const invoiceLineItemMutation = useMutation({
    mutationFn: updateInvoiceLineItems,
    onSuccess: () => {
      // Invalidate invoice line items queries
      queryClient.invalidateQueries({ queryKey: ['invoiceLineItems'] });
      // Also invalidate the invoice data since line items affect totals
      queryClient.invalidateQueries({ queryKey: ['invoices'] });
      queryClient.invalidateQueries({ queryKey: ['invoiceById', invoice.invoice_number] });
    }
  });

  const onSubmit = async (values: z.infer<typeof updateInvoiceFormSchema>) => {
    const { line_items, id, ...invoiceData } = values;
    const updatedInvoice = {
      ...
      issue_date: invoiceData.issue_date.toISOString().split('T')[0],
              ...
              <SheetClose asChild>
                <Button
                  ...
                  disabled={
                    ...
                    invoiceLineItemMutation.isPending
                  }>
                  {form.formState.isSubmitting ||
                  invoiceMutation.isPending ||
                  invoiceLineItemMutation.isPending

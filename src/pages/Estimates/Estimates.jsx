                    ...
                    <HStack mt={'1rem'} mb={'2rem'}>
                        ...
                        <Flex pr='1rem' mr={'1rem'} justifyContent={'end'} gap={10}>
                            ...
                            <Flex gap={4}>
                                ...
                                <Tooltip label='Create New Estimate'><IconButton colorScheme='blue' variant='solid' onClick={onNewOpen} icon={<MdPostAdd/>}/></Tooltip>
                            </Flex>
                        </Flex>
                    </HStack>
                                ...
                                <Tbody>
                                    {estimates?.map((estimate, index) => (
                                        <Tr key={estimate.id}>
                                            ...
                                            <Td textAlign={'center'}><Badge w={'80px'} variant={'subtle'} mx={'auto'} colorScheme={estimate.estimate_status.name === 'New' ? 'green' : '' || estimate.estimate_status.name === 'Accepted' ? 'green' : '' || estimate.estimate_status.name === 'Pending' ? 'yellow' : '' || estimate.estimate_status.name === 'Rejected' ? 'red' : 'gray'} p='1' rounded={'xl'} align='center'>{estimate.estimate_status.name}</Badge></Td>
                                            {/* <Td textAlign={'center'}>{estimate.estimate_status.name === 'Sent'? <><Text color={'white'} mx={'auto'} bg={'yellow.500'} p='1' rounded={'xl'} align='center' w={'80px'}>Sent</Text></>: 'false'}</Td> */}
                                            <Td><Text>{estimate.service_type.name}</Text></Td>
                                            <Td><Text>{ estimate.estimate_date ? new Date(estimate.estimate_date).toLocaleDateString(): ''}</Text></Td>
                                            <Td textAlign={'center'}><Text>{ estimate.issued_date ? new Date(estimate.issued_date).toLocaleDateString(): ''}</Text></Td>
                                            <Td><Text>{ estimate.expiration_date ? new Date(estimate.expiration_date).toLocaleDateString() : ''}</Text></Td>
                                            <Td>{estimate.customer.first_name && estimate.customer.last_name ? <><Flex><Link to={`/editcustomer/${estimate.customer.id}`}><Button variant={'ghost'} colorScheme={'facebook'}><Avatar size={'xs'} mr={'8px'} my={'auto'} /><Flex flexDir={'column'}><Flex fontWeight={'bold'} fontSize={'xs'}><Text marginRight={'4px'}>{estimate.customer.first_name}</Text><Text>{estimate.customer.last_name}</Text></Flex><Flex mt={'4px'} fontWeight={'light'} fontSize={'xs'}>{estimate.customer.email}</Flex></Flex></Button></Link></Flex></> : <>{estimate.customer.company_name}</>}</Td>
                                            ...
                                            <Td ><Text>${estimate.total ? (estimate.total).toLocaleString(undefined, {minimumFractionDigits : 2}) : '0'}</Text></Td>
                                            <Td><Flex gap={2}><Tooltip label='Edit'><IconButton icon={<MdEdit/>}/></Tooltip><Tooltip label='Delete'><IconButton onClick={() => {handleDelete(estimate.id, estimate.estimate_number)}} icon={<MdDelete/>}/></Tooltip><Link to={`/editestimate/${estimate.id}`}><Tooltip label='Go to Estimate Details '><IconButton colorScheme={'gray'} variant='solid' icon={<MdKeyboardArrowRight/>}/></Tooltip></Link></Flex></Td>
                                        </Tr>
